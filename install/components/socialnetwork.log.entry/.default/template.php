				...
				?><div class="<?=$separatorClassName?>" id="log_entry_inform_<?=intval($arEvent["EVENT"]["ID"])?>"><div class="feed-post-informers-cont"><?
					...
					{    if ($USER->IsAdmin()):
                        ...
                        <span href="javascript:void(0)" data-log-entry-url="<?=$strLogEntryURL?>"
                              onclick="BX.ready(function() {ebola_fill_socnetlog(<?=$arEvent["EVENT"]["ID"]?>, 'CHANGE');});"
                        >
                            <a class="feed-inform-follow"
                               id="ebola_fill_socnetlog_text_<?=$arEvent["EVENT"]["ID"]?>"></a>
                        </span>

                        <script type="text/javascript">
                            BX.ready(function() {ebola_fill_socnetlog(<?=$arEvent["EVENT"]["ID"]?>, 'CHECK');});
                        </script>
                    <?endif;?>
						<?if (
							is_set($arEvent)
