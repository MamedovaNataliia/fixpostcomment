		...
		<div class="<?=$className?>" id="blg-post-<?=$arResult["Post"]["ID"]?>">
			...
			<div class="feed-post-cont-wrap<?=$aditStyles?>" id="blg-post-img-<?=$arResult["Post"]["ID"]?>">
				...
				?><div class="<?=$separatorClassName?>" id="blg-post-inform-<?=$arResult["Post"]["ID"]?>"><div class="feed-post-informers-cont"><?
					...
					{
						?>
                    <span class="feed-inform-follow" data-follow="<?=($arParams["FOLLOW"] == "Y" ? "Y" : "N")?>
                       " id="log_entry_follow_<?=intval($arParams["LOG_ID"])?>" onclick="__blogPostSetFollow(<?=intval($arParams["LOG_ID"])?>)">
                        <a href="javascript:void(0);"><?=GetMessage("BLOG_POST_FOLLOW_".($arParams["FOLLOW"] == "Y" ? "Y" : "N"))?></a>
                    </span>
                    ...
                    <? if ($USER->IsAdmin()):?>
                        <script type="text/javascript">
                           /* function ebola_fill_socnetlog(id, action) {
                                BX.ajax({
                                    ...
                                    onsuccess: function (data) {
                                        BX("ebola_fill_socnetlog_text_" + id).innerHTML = data;
                                    }
                                });
                            }*/
                        </script>
                        <span href="javascript:void(0)" data-log-entry-url="<?=$strLogEntryURL?>"
                          onclick="BX.ready(function() {ebola_fill_socnetlog(<?=$arParams["LOG_ID"]?>, 'CHANGE');});">
                            <a class="feed-inform-follow"
                               id="ebola_fill_socnetlog_text_<?=$arParams["LOG_ID"]?>"></a>
                        </span>

                        <script type="text/javascript">
                            BX.ready(function() {ebola_fill_socnetlog(<?=$arParams["LOG_ID"]?>, 'CHECK');});
                        </script>
                    <?endif;?>
                        <a id="feed-post-menuanchor-<?=$arResult["Post"]["ID"]?>" href="#" class="feed-post-more-link">
                            <span class="feed-post-more-text" id="feed-post-more-<?=$arResult["Post"]["ID"]?>"><?=GetMessage("BLOG_POST_BUTTON_MORE")?></span>
                            <span class="feed-post-more-arrow"></span></a><?
					?><script>
